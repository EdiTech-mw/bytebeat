...
this.idx=0;
	...
	sample() {
		...
		if(v==null) return null;
		this._p();
		this.wavetable[this.idx+1&255]=this.wavetable[this.idx+1&255]*(9/10)+this.wavetable[this.idx&255]/10,
		this.idx++;
...
function main(t,SR) {
	...
	if(instruments==null && (round(t*SR)&255)==0) throw `set instruments to ${voices.length+1}`;
	lastT = round(t*SR);
	return mixer(4);
}
