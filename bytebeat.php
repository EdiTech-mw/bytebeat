        ...
        return '<fieldset>
			...
			<form name="form_editsong" method="post" action="?editsong">
				<table class="table-form"><tbody>
					<tr>
						<th>Hash</th>
						<td><input type="text" name="hash" value="' .
            $hash .
            '" readonly></td>
					</tr>
					<tr>
            ...
            '"></td>
					...
					<tr>
						...
						<td><input type="text" name="cover_url" value="' .
            $song["cover_url"] .
            '"></td>
					</tr>
					<tr>
						<th>Code original</th>
						<td><textarea name="code">' .
            $song["code"] .
            '</textarea></td>
					</tr>
					<tr>
						<th>Code minified</th>
						<td><textarea name="code_minified">' .
            $song["code_minified"] .
            '</textarea></td>
					</tr>
					<tr>
						<th>Code formatted</th>
						<td><textarea name="code_formatted">' .
            $song["code_formatted"] .
            '</textarea></td>
					</tr>
					<tr>
						...
						<td>
							<select name="drawing_mode">
								<option value=""' .
            ($drawing_mode ? " selected" : "") .
            '>None</option>
								<option value="Points"' .
            ($drawing_mode === "Points" ? " selected" : "") .
            '>Points</option>
								<option value="Waveform"' .
            ($drawing_mode === "Waveform" ? " selected" : "") .
            '>Waveform</option>
								<option value="Diagram"' .
            ($drawing_mode === "Diagram" ? " selected" : "") .
            '>Diagram</option>
								<option value="Combined"' .
            ($drawing_mode === "Combined" ? " selected" : "") .
            '>Combined</option>
							</select>
            ...
            '"></td>
					...
					<tr>
						...
						<td><select name="rating">
							<option value="0"' .
            ($song["rating"] === "0" ? " selected" : "") .
            '>No rating</option>
							<option value="1"' .
            ($song["rating"] === "1" ? " selected" : "") .
            '>Star</option>
							<option value="2"' .
            ($song["rating"] === "2" ? " selected" : "") .
            '>Yellow star</option>
						</select></td>
					</tr>
				</tbody></table>
...
function decodeLibraryFile($dbLink, $libName)
{
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        fancyDie(
            'File "' . $libFileName . '" has an error: ' . json_last_error_msg()
        );
    }
        ...
        foreach ($songs as $song) {
            $url = isset($song->url)
                ? (is_array($song->url)
                    ...
                    : $song->url)
                : null;
            $codeOriginal = isset($song->code)
                ? $song->code
                : (isset($song->fileOrig)
                    ? file_get_contents(
                        $songsPath . "original/" . $song->hash . ".js"
                    )
                    : null);
            $codeMinified = isset($song->codeMin)
                ? $song->codeMin
                : (isset($song->fileMin)
                    ? file_get_contents(
                        $songsPath . "minified/" . $song->hash . ".js"
                    )
                    : null);
            $codeFormatted = isset($song->fileForm)
                ? file_get_contents(
                    $songsPath . "formatted/" . $song->hash . ".js"
                )
                : null;
            ...
            mysqli_query(
                ...
                "INSERT INTO `songs` (hash" .
                    ...
                    (isset($song->stereo) ? ", 1" : "") .
                    (isset($codeOriginal)
                        ? ', "' . addslashes($codeOriginal) . '"'
                        : "") .
                    (isset($codeMinified)
                        ? ', "' . addslashes($codeMinified) . '"'
                        : "") .
                    (isset($codeFormatted)
                        ? ', "' . addslashes($codeFormatted) . '"'
                        : "") .
                    (isset($song->coverName)
                        ? ', "' . addslashes($song->coverName) . '"'
                        : "") .
                    (isset($song->coverUrl)
                        ? ', "' . $song->coverUrl . '"'
                        : "") .
...
function getDBLink()
{
    ...
    if (!$dbLink) {
        fancyDie(
            "Could not connect to database: " .
                (is_object($dbLink)
                    ? mysqli_error($dbLink)
                    : (($dbLinkError = mysqli_connect_error())
                        ? $dbLinkError
                        : "(unknown error)"))
        );
    }
    ...
    mysqli_query($dbLink, "SET collation_connection = 'utf8mb4_general_ci'");
...
function filesToDatabase()
{
    ...
    if (
        mysqli_num_rows(mysqli_query($dbLink, 'SHOW TABLES LIKE "songs";')) ===
        0
    ) {
        mysqli_query(
            $dbLink,
    ...
    $message .=
        "Libraries are copied into the `songs` and `remixes` database tables.<br>";
...
function databaseToFiles()
{
    ...
    while ($song = mysqli_fetch_assoc($qResult)) {
        ...
        $fileForm = 0;
        if (
            isset($song["code_minified"]) &&
            mb_strlen($song["code_minified"]) > 1024
        ) {
            // Save big minified code into file
            file_put_contents(
                $pathMinified . $song["hash"] . ".js",
                $song["code_minified"]
            );
            $fileMin = 1;
        }
        if (
            isset($song["code"]) &&
            (substr_count($song["code"], PHP_EOL) > 4 ||
                mb_strlen($song["code"]) > 1024)
        ) {
            // Save big original code into file
            file_put_contents(
                $pathOriginal . $song["hash"] . ".js",
                $song["code"]
            );
            $fileOrig = 1;
        }
            ...
            if (mysqli_num_rows($qSource) !== 0) {
                while ($source = mysqli_fetch_assoc($qSource)) {
                    $remixStr .=
                        ...
                        '"' .
                        (isset($source["author"])
                            ? ', "author":' . json_encode($source["author"])
                            : "") .
                        (isset($source["name"])
                            ? ', "name":' . json_encode($source["name"])
                            : "") .
                        (isset($source["url"])
                            ? ', "url":' . json_encode($source["url"])
                            : "") .
                        "}";
                }
            }
        ...
        $songsByHash[$song["hash"]] =
            '{"hash":"' .
            $song["hash"] .
            '"' .
            (isset($song["name"])
                ? ',"name":' . json_encode($song["name"])
                : "") .
            (isset($song["description"])
                ? ',"description":' . json_encode($song["description"])
                : "") .
            (isset($song["url"])
                ? ',"url":' .
                    (str_starts_with($song["url"], "[")
                        ? $song["url"]
                        : '"' . $song["url"] . '"')
                : "") .
            ...
            ($fileOrig
                ? ',"fileOrig":1'
                : (isset($song["code"])
                    ? ',"code":' . json_encode($song["code"])
                    : "")) .
            (isset($song["code"])
                ? ',"codeLen":' . strlen($song["code"])
                : "") .
            ($fileMin
                ? ',"fileMin":1'
                : (isset($song["code_minified"])
                    ...
                    : "")) .
            (isset($song["code_minified"])
                ? ',"codeMinLen":' . strlen($song["code_minified"])
                : "") .
            ($fileForm
                ? ',"fileForm":1,"codeFormLen":' .
                    strlen($song["code_formatted"])
                : "") .
            ($remixStr ? ',"remix":[' . $remixStr . "]" : "") .
            (isset($song["cover_name"])
                ? ',"coverName":' . json_encode($song["cover_name"])
                : "") .
            (isset($song["cover_url"])
...
function addSong($isEdit)
{
    ...
    $codeFormatted = str_replace(
        "\r",
        "",
        addslashes($_POST["code_formatted"])
    );
    ...
    $drawingScale =
        $drawingScale && is_numeric($drawingScale) ? $drawingScale : null;
    $drawing =
        $drawingMode || isset($drawingScale)
            ? addslashes(
                "{" .
    ...
    if ($isEdit) {
        ...
        $codeLen = $_POST["code"]
            ? strlen($_POST["code"])
            : ($_POST["code_formatted"]
                ? strlen($_POST["code_formatted"])
                : ($_POST["code_minified"]
                    ? strlen($_POST["code_minified"])
                    : 0));
        if ($codeLen) {
            if ($codeLen <= 256) {
                $tagsArr[] = "256";
    ...
    if ($isEdit) {
        ...
        mysqli_query(
			...
			`hash` = "' .
                ...
                $sampleRate .
                ", `stereo` = " .
                (isset($_POST["stereo"]) ? 1 : "NULL") .
                ", `code` = " .
                ($code ? '"' . $code . '"' : "NULL") .
                ", `code_minified` = " .
                ($codeMinified ? '"' . $codeMinified . '"' : "NULL") .
                ", `code_formatted` = " .
                ($codeFormatted ? '"' . $codeFormatted . '"' : "NULL") .
                ", `cover_name` = " .
